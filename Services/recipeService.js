...
module.exports = function (app, supabase) {
    ...
    app.post('/api/recipes', async (req, res) => {
      try {
        ...
        if (!name || !ingredients || !instructions) return res.status(400).json({ error: 'Missing required fields' });
    ...
    app.delete('/api/recipes/:id', async (req, res) => {
      try {
        ...
        const { error: checkError } = await supabase.from('recipes').select('id').eq('id', id).single();
        if (checkError) return res.status(404).json({ error: 'Recipe not found' });
